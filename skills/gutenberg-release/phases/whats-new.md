...
5. **Follow the usual Make Core post structure.** The writer has creative leeway, but default to the structure used by recent "What's new in Gutenberg" posts:
   - Title: `What's new in Gutenberg X.Y? (DD Mon)`.
   - Opening boilerplate: one short paragraph explaining that these posts follow every Gutenberg release and showcase new features, plus the standard reminder link to ways to keep up with Gutenberg and the Editor when available.
   - Release sentence: `Gutenberg X.Y has been released and is available for download!` with the plugin/download link.
   - Overview: 1–3 short paragraphs summarizing the most important themes before the table of contents.
   - Table of contents.
   - 2–4 main highlight sections with prose and PR links.
   - `Other Notable Highlights` as a compact bullet list for secondary user/developer-facing items.
   - `Changelog` with the generated release-note categories preserved where practical.
   - `First-time contributors` and `Contributors`, copied/refreshed from the final release notes.
6. **Draft the post as block markup, not Markdown** — the user pastes it into the Make Core editor. Prefer writing the draft to `.context/gutenberg-X.Y-whats-new-draft.html` and a companion source audit to `.context/gutenberg-X.Y-whats-new-sources.md` so the user can review and other agents can continue the work:
   ```html
   <!-- wp:heading {"level":2} -->
   ...
   <!-- /wp:paragraph -->
   ```
   If producing a full first draft in one pass, keep claims conservative and mark anything not deeply checked in the source audit.
7. **Pin every claim to a real source.** Each highlight's title and description must come verbatim (or faithfully paraphrased) from the originating issue/PR — **never invent a feature description, author name, or behavior.** Cross-check each PR's actual title and body before showing a section, and surface any drift:
   ```
...
8. **Prepare the Make Core handoff.** Give the user the post title, the `.context` draft path, and any media/screenshot URLs that should be uploaded or embedded. Do not guess Make Core taxonomy, publish date, excerpt, or author metadata; ask the user if those are needed. Remind them to paste block markup into the code editor/block editor, preview the post, and check headings, links, images, and changelog freshness.
9. **Refresh before publishing.** Before the final stable post goes live, refresh from the stable release notes and any post-RC backports. Do not present the RC changelog as final if stable has not shipped yet.
10. **Post for review** in `#core` (w.org), optionally `#design` (w.org) and `#core` (a8c). Draft each Slack message in a fenced block for the user to paste. Include the Make Core draft URL if the user provides it.
